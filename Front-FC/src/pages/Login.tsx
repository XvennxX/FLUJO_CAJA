...
const Login: React.FC = () => {
  ...
  // Log para debugging del estado del error
  console.log('🎨 Login render - loginError:', loginError, 'isLoading:', isLoading);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('🚀 Calling login function (Login.tsx)...');
      const result = await login(email, password);
      console.log('📨 Login result (Login.tsx):', result);
      
      if (result.success) {
        console.log('✅ Login exitoso (Login.tsx)');
      } else {
        console.log('❌ Login falló (Login.tsx):', result.error);
        console.log('🔴 Error manejado en contexto...');
      }
    } catch (err) {
      console.error('💥 Error en login (Login.tsx):', err);
