...
def main():
    ...
    try:
        ...
        logger.info(f"   Encontradas {len(cuentas)} cuentas")
        for cuenta in cuentas:
            logger.info(f"   - Cuenta {cuenta.id}: {cuenta.numero_cuenta}")
        
        # 2. Verificar concepto SUB TOTAL TESORERÍA
        logger.info("\n2. Verificando concepto SUB TOTAL TESORERÍA...")
        concepto_subtotal = db.query(ConceptoFlujoCaja).filter(
            ConceptoFlujoCaja.id == 50
        ).first()
        
        if concepto_subtotal:
            logger.info(f"   ✓ Concepto encontrado: {concepto_subtotal.nombre}")
            logger.info(f"   ✓ Fórmula: {concepto_subtotal.formula_dependencia}")
        else:
        ...
        for cuenta in cuentas:
            ...
            count_tesoreria = db.query(TransaccionFlujoCaja).filter(
            ...
            subtotal_existe = db.query(TransaccionFlujoCaja).filter(
                TransaccionFlujoCaja.fecha == fecha_actual,
                TransaccionFlujoCaja.concepto_id == 50,
                TransaccionFlujoCaja.cuenta_id == cuenta.id
            ).first()
            
            logger.info(f"   Cuenta {cuenta.numero_cuenta}: {count_tesoreria} transacciones tesorería, "
        ...
        for cuenta in cuentas:
            logger.info(f"\n   Procesando cuenta {cuenta.numero_cuenta} (ID: {cuenta.id})...")
            ...
            if transaccion_existente:
                ...
                nueva_transaccion = TransaccionFlujoCaja(
                    ...
                    area="tesoreria",  # Cambiar de "SISTEMA" a "tesoreria" para coincidir con ENUM
                    compania_id=1,
                    auditoria={
            ...
            logger.info(f"      ✓ Procesada cuenta {cuenta.numero_cuenta}")
        ...
        for cuenta in cuentas:
            ...
            if subtotal:
                logger.info(f"   ✓ Cuenta {cuenta.numero_cuenta}: SUB TOTAL = ${subtotal.monto:,.2f}")
            else:
                logger.warning(f"   ⚠ Cuenta {cuenta.numero_cuenta}: Sin SUB TOTAL")
